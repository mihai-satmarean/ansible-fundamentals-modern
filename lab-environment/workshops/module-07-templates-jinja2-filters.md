...
<body>
    ...
    <div class="filter-example">
        ...
        <div class="result">Custom format: {{ ansible_date_time.epoch | int | strftime('%B %d, %Y at %I:%M %p') }}</div>
    </div>
    
    <h2>IP Address Filters</h2>
    <div class="filter-example">
        <div class="original">IP Address: {{ ansible_default_ipv4.address }}</div>
        <div class="result">Network: {{ ansible_default_ipv4.address | ansible.utils.ipaddr('network') }}</div>
        <div class="result">Netmask: {{ ansible_default_ipv4.address | ansible.utils.ipaddr('netmask') }}</div>
...
ansible-playbook -i hosts.yml basic_templates.yml

# Check the deployed pages
curl http://51.20.81.242/
```
...
ansible-playbook -i hosts.yml advanced_templates.yml

# Check the filter demonstration page
curl http://51.20.81.242/filters.html
```
...
ansible-playbook -i hosts.yml complex_templates.yml

# Check generated configuration
ansible webservers -i hosts.yml -m shell -a "head -20 /etc/{{ inventory_hostname }}-config.conf"
```
